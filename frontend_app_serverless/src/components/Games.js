        ...
        <div className='grid grid-cols-4 gap-4'>
            {
                games.map((game, index) => (
                    <div key={index} className='p-8 flex flex-col items-center'>
                        <img src={game.game_art_url} alt={`Art for ${game.game_title}`} className='w-36 h-36 rounded-2xl' />
                        <h2 className="font-bold">{game.game_title}</h2>
                        <p>Release Date: {game.release_date}</p>
                        <Link className="text-black italic underline" href={`/game/${game.url_path}`}>Read Reviews</Link>
                    </div>
                ))
            }
        </div>

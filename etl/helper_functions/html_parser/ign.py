...
class IGNGameReviewParser(AbstractHTMLParser):
    ...
    def get_reviewer_name(self):
        try:
            author_tag = self.soup.find('a', class_="authorByline__name")
    ...
    def get_critic_score(self):
        try:
            score_tag = self.soup.find('figcaption', class_="review-score-wrapper")
            score = score_tag.get_text().strip() if score_tag else None
            return int(float(score) * 10) if score else None
        except AttributeError as e:
